...
class Api:
    ...
    def load_cookie(self):
        ...
        with open("user_data.json", "r") as r:
            ...
            if self.user_data["specificID"]:
                self.user_data["username"], self.headers["Cookie"] = j[self.user_data["specificID"]][0], \
                                                                     j[self.user_data["specificID"]][1]
            else:
                j = j[list(j.keys())[0]]
    ...
    def orderInfo(self):
        ...
        self.user_data["screen_id"], self.user_data["sku_id"], self.user_data["pay_money"] = self.menu("GET_ORDER_IF",
                                                                                                       data["data"])
        # exit(0)
        # exit(0)
    ...
    def tokenGet(self):
        ...
        payload = "count=" + str(self.user_data["user_count"]) + "&order_type=1&project_id=" + self.user_data[
            "project_id"] + "&screen_id=" + str(self.user_data["screen_id"]) + "&sku_id=" + str(
            self.user_data["sku_id"]) + "&token="
    ...
    def orderCreate(self):
        ...
        if self.user_data["auth_type"] == 0:
            payload = {
                ...
                "deliver_info": {"name": addr_dict['姓名'], "tel": addr_dict['电话'], "addr_id": addr_dict['地址id'], "addr": addr_dict['地址']},
                "pay_money": int(self.user_data["pay_money"]) * int(self.user_data["user_count"]) + 1000,
    ...
    def menu(self, mtype, data=None):
        if mtype == "GET_SHOW":
            ...
            try:
                ...
                self.error_handle("请输入正确数字")
            self.selectedTicketInfo = data["name"] + " " + data["screen_list"][date]["name"] + " " + \
            ...
            return data["screen_list"][date]["id"], data["screen_list"][date]["ticket_list"][choice]["id"], \
                   data["screen_list"][date]["ticket_list"][choice]["price"]
